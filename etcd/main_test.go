...
/**
 * The purpose of this "test" is to run etcd etcd with code-coverage
...
func TestMain(m *testing.M) {
	// don't launch etcd etcd when invoked via go test
	if strings.HasSuffix(os.Args[0], ".test") {
		log.Printf("skip launching etcd etcd when invoked via go test")
		return
	}
