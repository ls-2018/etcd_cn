...
import (
	...
	"github.com/ls-2018/etcd_cn/etcd_backend/mock/mockstorage"
	"github.com/ls-2018/etcd_cn/etcd_backend/mock/mockstore"
	"github.com/ls-2018/etcd_cn/etcd_backend/mock/mockwait"
...
// TestDoBadLocalAction tests etcd requests which do not need to go through consensus,
...
// TestApplyRepeat tests that etcd handles repeat raft messages gracefully
...
// TestSyncTrigger tests that the etcd proposes a SYNC request when its sync timer ticks
func TestSyncTrigger(t *testing.T) {
	...
	// trigger the etcd to become a leader and accept sync requests
	go func() {
		srv.start()
		n.readyc <- raft.Ready{
...
// TODO: test etcd could stop itself when being removed
...
// TestPublishStopped tests that publish will be stopped if etcd is stopped.
