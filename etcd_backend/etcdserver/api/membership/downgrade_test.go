...
func TestMustDetectDowngrade(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"Succeeded when downgrade is disabled and cluster version is etcd version",
			lv,
			downgradeDisabled,
			true,
			"",
		...
		{
			"Failed when downgrade is disabled and etcd version is lower than determined cluster version ",
			oneMinorHigher,
			downgradeDisabled,
			false,
			"invalid downgrade; etcd version is lower than determined cluster version",
		},
		{
		...
		{
			"Failed when downgrade is enabled and etcd version is target version",
			lv,
			downgradeEnabledEqualVersion,
			true,
		...
		{
			"Succeeded when downgrade to lower version and etcd version is cluster version ",
			lv,
			downgradeEnabledLowerVersion,
			false,
			"invalid downgrade; etcd version is not allowed to join when downgrade is enabled",
		},
		{
