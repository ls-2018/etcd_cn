...
func TestUnmarshalSuccessfulResponse(t *testing.T) {
	...
	tests := []struct {
		...
		// Neither PrevNode or RaftNodeInterFace
		{
			indexHdr: "1",
			body:     `{"action":"delete"}`,
		...
		// PrevNode and RaftNodeInterFace
		{
			indexHdr: "15",
	...
	for i, tt := range tests {
		...
		if !reflect.DeepEqual(res.Node, tt.wantRes.Node) {
			t.Errorf("#%d: RaftNodeInterFace=%v, expected %v", i, res.Node, tt.wantRes.Node)
		}
	}
}
